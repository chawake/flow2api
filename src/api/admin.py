...
class AddTokenRequest(BaseModel):
    st: str
    project_id: Optional[str] = None  # User optional input for project_id
...
async def get_tokens(token: str = Depends(verify_admin_token)):
    ...
    for t in tokens:
        ...
        result.append({
            ...
            "token": t.at,  # Compatible with frontend token.token access
            "email": t.email,
            "name": t.name,
            "remark": t.remark,
...
async def add_token(
    ...
    try:
        new_token = await token_manager.add_token(
            st=request.st,
            project_id=request.project_id,  # 🆕 Supports user-specified project_id
...
async def refresh_at(
    ...
    try:
        # Call internal refresh method of token_manager
    ...
    for idx, item in enumerate(request.tokens):
        try:
            ...
            try:
                ...
                # Check if it already exists using email
...
async def update_token_refresh_enabled(
    ...
    """Update AT auto refresh enabled (Flow2API is permanently enabled, this endpoint is for frontend compatibility only)"""
    return {
        "success": True,
